...
@bot.tree.command(name="track_file",
                  description="Add a repository to tracking list")
async def track_file(interaction: discord.Interaction, author: str,
                     repo_name: str):
    ...
    if path in data:
        await interaction.response.send_message(
            f"{fileUrl} is already tracking list")
        return
...
@bot.tree.command(name="untrack_file",
                  description="Remove a file from tracking list")
async def untrack_file(interaction: discord.Interaction, author: str,
                       repo_name: str):
    ...
    try:
        ...
        saveData(DATA_FILE, data)
        await interaction.response.send_message(
            f"Removed {fileUrl} from tracking list")
    #If repo doesn't exist, send error message
    except:
        await interaction.response.send_message(
            "The repository is not in the tracking list")


@bot.tree.command(name="tracking_list",
                  description="Prints list of repositories being tracked")
